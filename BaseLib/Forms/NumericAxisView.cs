...
namespace BaseLib.Forms{
	...
	public class NumericAxisView : BasicView{
		...
		public ConfigurableType Configurable{ get; set; }
		...
		private void PaintIndicator(IGraphics g, int width, int height){
			...
			if (IsHorizontal()){
				if (len >= 1){
					...
					g.DrawLine(indicatorPen, GetMinIndicator(GetLength(width, height)), 0,
						GetMinIndicator(GetLength(width, height)), height - 1);
				}
			} else{
				if (len >= 1){
		...
		public void OnPaint(IGraphics g, int xm, int ym, int width, int height){
			...
			double max = 0;
			if (IsValid()){
				decade = (int) Math.Floor(Math.Log(Math.Max(Math.Abs(VisibleMax), Math.Abs(VisibleMin))) /
				                          Math.Log(10));
					...
					if (IsValid() && tics != null){
						...
						for (int i = 0; i < tics[0].Length; i++){
							...
							if (Sign * pos > Sign * previousstringEnd){
								DrawString(g, s, numbersFont, brush, xm + pos + 1,
									ym + y0 - MajorTickLength - numbersFont.Height);
					...
					if (IsValid() && tics != null){
						...
						for (int i = 0; i < tics[0].Length; i++){
							...
							if (Sign * pos > Sign * previousstringEnd){
								...
								DrawString(g, s, numbersFont, brush, -pos - ym,
									xm + x0 - MajorTickLength - numbersFont.Height);
								g.RotateTransform(90);
								previousstringEnd = pos + Sign * w;
							}
						}
				...
				case AxisPositioning.Right:
					...
					if (IsValid() && tics != null){
						...
						for (int i = 0; i < tics[0].Length; i++){
							...
							if (Sign * pos > Sign * previousstringEnd){
								g.RotateTransform(90);
								DrawString(g, s, numbersFont, brush, ym + pos + 1,
									-xm - MajorTickLength - numbersFont.Height * 0.99f);
								g.RotateTransform(-90);
								previousstringEnd = pos + Sign * w;
							}
						}
		...
		public void TransformAxis(){
			NumericAxisPropertiesForm f = new NumericAxisPropertiesForm(Text,
				IsLogarithmic ? Math.Exp(ZoomMin) : ZoomMin, IsLogarithmic ? Math.Exp(ZoomMax) : ZoomMax);
			f.ShowDialog();
			if (f.Ok){
				Text = f.Title;
		...
		public override void OnMouseIsUp(BasicMouseEventArgs e){
			if (Configurable != ConfigurableType.None && !e.IsMainButton){
				NumericAxisPropertiesForm f = new NumericAxisPropertiesForm(Text,
					IsLogarithmic ? Math.Exp(ZoomMin) : ZoomMin, IsLogarithmic ? Math.Exp(ZoomMax) : ZoomMax);
				f.ShowDialog();
				if (f.Ok){
					...
					if (!double.IsNaN(newMin) && !double.IsNaN(newMax) && newMin < newMax){
						ZoomMin = newMin;
						ZoomMax = newMax;
						if (Configurable == ConfigurableType.Total){
							TotalMin = newMin;
							TotalMax = newMax;
						}
						FireZoomChanged();
