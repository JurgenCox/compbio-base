...
namespace BaseLibS.Graph
{
    public static class GraphUtil
    {
        ...
        private static (int x, int y ) PosMoveUpButton(int width, int height, int bsize, int p) =>
            (width - 3 * bsize, height - 2 * (bsize + p));

        private static (int x, int y ) PosMoveLeftButton(int width, int height, int bsize, int p) => (
            (int) (width - 3.5 * bsize - p / 2), (int) (height - 1.5f * (bsize + p)));

        private static (int x, int y ) PosMoveRightButton(int width, int height, int bsize, int p) => (
            (int) (width - 2.5 * bsize + p / 2), (int) (height - 1.5f * (bsize + p)));

        private static (int x, int y ) PosMoveDownButton(int width, int height, int bsize, int p) =>
            (width - 3 * bsize, height - bsize - p);

        public static void PaintMoveButtons(IGraphics g, int width, int height, MoveButtonState state, double? f = null)
        {
            int bsize = GraphUtil.zoomButtonSize;
            int p = 5;
            ...
            var t = PosMoveUpButton(width, height, bsize, p);
            PaintMoveUpButton(g, b, t.x, t.y, bsize);
            b = buttonBrush;
            switch (state)
            {
            ...
            t = PosMoveLeftButton(width, height, bsize, p);
            PaintMoveLeftButton(g, b, t.x, t.y, bsize);
            b = buttonBrush;
            switch (state)
            {
            ...
            t = PosMoveRightButton(width, height, bsize, p);
            PaintMoveRightButton(g, b, t.x, t.y, bsize);
            b = buttonBrush;
            switch (state)
            {
            ...
            t = PosMoveDownButton(width, height, bsize, p);
            PaintMoveDownButton(g, b, t.x, t.y, bsize);
            g.SmoothingMode = SmoothingMode2.Default;
        }

        
        private static (int x, int y ) PosPlusZoomButton(int width, int height, int bsize, int p) =>
            (width - bsize - p, height - 2 * (bsize + p));
        private static (int x, int y ) PosMinusZoomButton(int width, int height, int bsize, int p) =>
            (width - bsize - p, height - bsize - p);
        public static void PaintZoomButtons(IGraphics g, int width, int height, ZoomButtonState state, double? f = null)
        {
            int bsize = GraphUtil.zoomButtonSize;
            int p = 4;
            ...
            var t = PosPlusZoomButton(width, height, bsize, p);
            PaintPlusZoomButton(g, b, t.x, t.y, bsize);
            b = buttonBrush;
            switch (state)
            {
            ...
            t = PosMinusZoomButton(width, height, bsize, p);
            PaintMinusZoomButton(g, b, t.x, t.y, bsize);
            g.SmoothingMode = SmoothingMode2.Default;
        }


        public static void PaintMoveUpButton(IGraphics g, Brush2 b, int x, int y, int bsize, double? f = null)
        {
            ...
            g.FillClosedCurve(Brushes2.White,
                new[]
                {
                    new Point2(x + bsize / 2, y + bsize / 4 * 1),
                    new Point2(x + bsize / 4 * 3, y + bsize / 4 * 3),
                    new Point2(x + bsize / 4 * 1, y + bsize / 4 * 3)
                });
        }
        ...
        public static void PaintMoveDownButton(IGraphics g, Brush2 b, int x, int y, int bsize)
        {
            ...
            g.FillClosedCurve(Brushes2.White,
                new[]
                {
                    new Point2(x + bsize / 2, y + bsize / 4 * 3),
                    new Point2(x + bsize / 4 * 3, y + bsize / 4 * 1),
                    new Point2(x + bsize / 4 * 1, y + bsize / 4 * 1)
                });
        }
        ...
        public static void PaintMoveLeftButton(IGraphics g, Brush2 b, int x, int y, int bsize)
        {
            ...
            g.FillClosedCurve(Brushes2.White,
                new[]
                {
                    new Point2(x + bsize / 4 * 1, y + bsize / 2),
                    new Point2(x + bsize / 4 * 3, y + bsize / 4 * 3),
                    new Point2(x + bsize / 4 * 3, y + bsize / 4 * 1)
                });
        }
        ...
        public static void PaintMoveRightButton(IGraphics g, Brush2 b, int x, int y, int bsize)
        {
            ...
            g.FillClosedCurve(Brushes2.White,
                new[]
                {
                    new Point2(x + bsize / 4 * 3, y + bsize / 2),
                    new Point2(x + bsize / 4 * 1, y + bsize / 4 * 3),
                    new Point2(x + bsize / 4 * 1, y + bsize / 4 * 1)
                });
        }
            ...
            double? f = null)
        {
            ...
            Size2 overview = CalcOverviewSize(visibleWin.Width, visibleWin.Height, totalSize.Width, totalSize.Height,
                f == null ? maxOverviewSize : (int) (f * maxOverviewSize));
            ...
            {
                g.FillRectangle(b, xpos + win.X, ypos + win.Y + win.Height - 1, win.Width,
        ...
        public static Size2 CalcOverviewSize(int width, int height, int totalWidth, int totalHeight,
            int sizeLimit = maxOverviewSize)
        {
            int maxSize = Math.Min(Math.Min(sizeLimit, height), width - 20);
        ...
        public static bool HitsAMoveButton(int x, int y, int width, int height, double? f) => GetNewMoveButtonState(x, y, width, height, true, f) == MoveButtonState.Neutral;

        public static bool HitsAZoomButton(int x, int y, int width, int height,double? f) => GetNewZoomButtonState(x, y, width, height,true, f) == ZoomButtonState.Neutral;

        private static bool HitsCircle(int x, int y, int cx, int cy, float d) =>
            (x - cx) * (x - cx) + (y - cy) * (y - cy) <= d * d * 0.5 * 0.5;
        ...
        public static ZoomButtonState GetNewZoomButtonState(int x, int y, int width, int height, bool press,double? f)
        {
            int bsize = GraphUtil.zoomButtonSize;
            int p = 5;
            ...
            var t = PosMinusZoomButton(width, height, bsize, p);
            if (HitsCircle(x, y, t.x, t.y, bsize))
                return press ? ZoomButtonState.PressMinus : ZoomButtonState.HighlightMinus;

            t = PosPlusZoomButton(width, height, bsize, p);
            if (HitsCircle(x, y, t.x, t.y, bsize))
        ...
        public static MoveButtonState GetNewMoveButtonState(int x, int y, int width, int height, bool press, double? f)
        {
            int bsize = GraphUtil.zoomButtonSize;
            int p = 5;
        ...
        public static void FillShadedRectangle(Bitmap2 b, int w, int h)
        {
            ...
            for (int i = 0; i < wi; i++)
            {
                int[][] pix = InterpolateRgb(upper[0][i], upper[1][i], upper[2][i], lower[0][i], lower[1][i],
                    lower[2][i], he);
                for (int j = 0; j < he; j++)
                {
            ...
            {
                b.SetPixel(i, h - 1,
                    Color2.FromArgb(pix2[0][i - w / 2 - 1], pix2[1][i - w / 2 - 1], pix2[2][i - w / 2 - 1]));
            }
        }
